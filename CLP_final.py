...
CLP_final.py
============
Final CLP estimator: KFold cross-fitting (plain, not grouped by person).
Economically motivated feature set (base 28 + econ interactions).
...
────────────
  ...
  "econ"  — 28 base + 227 economically motivated features = 255 total
             ...
             ── Squared terms (18) ──────────────────────────────────────
             Meaningful (10): ernpq1²..ernpq8², kidcount², yremp²
             Near-binary (8): adcpq1²..adcpq8²
             ...
             ── Dropped (Tier 3) ────────────────────────────────────────
             binary² (binary dummies squared — identical to original)
...
All estimators use K=5-fold cross-fitting (Semenova 2026, Def. 5.2).
...
# =============================================================================
# 11.  CROSS-FITTED FIRST STAGE  (plain KFold)
# =============================================================================
def estimate_b0_features(D, state_obs, X_base_raw, X_econ_raw,
                          feature_set, estimator_name,
                          K=5, seed=42, pscorewt=None,
                          X_rf_raw=None, X_disc_raw=None):
    """
    K-fold cross-fitted first stage.
    Fits model per B-component.
    ...
    for train_idx, test_idx in kf.split(X_base_raw):
...
def print_summary_table(all_results):
    ...
    means = []
    for name in config_names:
        widths = [v['width'] for v in all_results[name].values()]
    ...
    for fset in ("base", "econ", "rf", "disc"):
        ...
        if not matching:
            continue
        avg = np.mean([np.mean([v['width'] for v in all_results[m].values()])
                       for m in matching])
    ...
    print(f"\n  NOTE: K={K_FOLDS}-fold cross-fitting, N_BOOTSTRAP={N_BOOTSTRAP}.")
...
def main():
    ...
    print("=" * 68)
    print("CLP Final  —  LASSO / Ridge / OLS  (base vs. econ set, KFold)")
    print("=" * 68)
    print("Configurations (6 total, no RandomForest):")
