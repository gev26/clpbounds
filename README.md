...
Once $\hat b(X_i)$ is in hand, the CLP plug-in step requires solving $min_{\nu \in T_q} \nu' \hat b(X_i)$ for every observation $i$, where $T_q$ = { $\nu$ : $A^T \nu \geq q$ }. How this is done depends on the polytope size. In coarse granularity regimes exact min is plausible since vertex set is small, for example C(9,5) = 126 unique vertex candidates. But with more granular regimes this number is much higher.
...
**Per-component fitting.** All estimators are fit per B-component. Within each fold, a `StandardScaler`
...
- **TUWelf (Take-Up Welfare)** sums $\beta(0n,1r) + \beta(2n,1r) + \beta(1n,1r)$ — the lead columns of G1, G3, G7.
- **Exit 0r** is simply $\beta(0r, 0n)$ — the single column of G2.

  
  ### Summary table

`✓` = sub-bin split active; `−` = pooled at this granularity. The spec column is the numbering of specification used in the code, not in the paper.
...
- **Spec 18** is the most aggressive simplification: KT 5-row layout + two extra constraint rows (low_b1p, low_b2p) that point-identify $\beta$(2n, low_b1r) and $\beta$(2n, low_b2r) under the assumption that pooled-1r flows in G1/G5/G7/G9 land entirely in low_b3r.
...
- **`CLP_synthetic_simulation.py`**.
  ...
  bracket the true $\beta_0$, (ii) that they are tighter than the
...
- **`CLP_ols.py`**.
  ...
  covariate subsets (we only report full sample with baselines 28 covariates case in the paper). This replicates **Table 1** in the paper.
...
- **`CLP_granular.py`**.
  This replicates **Table 2** in the paper and **Table B.3** in the Appendix.
  ...
  **LASSO** estimator, two covariate sets (**base, extended**), under
  ...
  regimes corresponds to **Table 2** in the paper. **Table B.3** has all specificatoins with base covariate set and GroupKFold cross-fitting regime.
...
- **`CLP_welfare.py`**.
  This replicates **Table B.4** in the paper.
  Implements the welfare bounds with
  **LASSO** estimator, base covariate set, under
  GroupKFold cross-fitting by person, with different granularity designs. 
